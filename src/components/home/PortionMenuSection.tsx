  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all overflow-hidden flex flex-col">
      ...
      <div className="p-6 flex flex-col flex-1 bg-white">
        <div className="mb-3">
          <p className="text-xs font-bold uppercase tracking-wider text-[#005D33] mb-2">{categoryLabel}</p>
        ...
        <div className="space-y-3 mt-auto">
                      ...
                      return (
                        <span
                          ...
                          className={`inline-flex items-center rounded-full px-2.5 py-1 text-sm font-black ${
                            isActive ? 'bg-[#005D33] text-white' : 'bg-white text-amber-700 border border-amber-100'
                          }`}
                        >
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <button onClick={onAdd} className={`flex-1 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 ${justAdded ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-[#005D33]'}`}>
    ...
    <section id="portion-menu" className="w-full relative py-16 md:py-24 bg-white border-t border-gray-100">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-12 max-w-3xl mx-auto">
          <span className="inline-flex items-center gap-2 bg-[#F9A826]/15 text-[#005D33] px-4 py-1.5 rounded-full text-sm font-bold tracking-wide mb-4 uppercase">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20 gap-3 text-slate-500 font-bold">
            <Loader2 className="w-8 h-8 animate-spin text-[#005D33]" /> Memuat menu perporsi...
          </div>
        ) : (
          <>
            <div className="flex flex-wrap justify-center gap-2 mb-10">
              <button
                ...
                className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${selectedCategory === 'all' ? 'bg-[#005D33] text-white shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}
              >
                Semua
              </button>
              {categories.map(category => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${selectedCategory === category.slug ? 'bg-[#005D33] text-white shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}
                >
                  {category.name}
                </button>
              ))}
            ...
            <div className="space-y-16 max-w-7xl mx-auto">
              <div id="menu-satuan" className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-black text-[#005D33]">Menu Satuan</h3>
              ...
              <div id="menu-acara" className="space-y-6 bg-slate-50 border border-gray-100 rounded-[32px] p-5 md:p-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
                  <div>
                    <h3 className="text-2xl md:text-3xl font-black text-[#005D33]">Menu Acara & Borongan</h3>
