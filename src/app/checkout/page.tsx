    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex items-center justify-center">
        <div className="text-center space-y-4">
          <Loader2 className="w-10 h-10 animate-spin text-[#005D33] mx-auto" />
          <p className="text-lg font-semibold text-[#005D33]">Mengarahkan ke halaman pembayaran...</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        {/* Back */}
        <Link href="/" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-[#005D33] transition-colors mb-6">
          <ArrowLeft className="w-4 h-4" />
          Kembali ke Beranda
        </Link>

        <h1 className="text-3xl font-extrabold mb-8 text-[#005D33]">Checkout</h1>

        {items.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-slate-500 text-lg">Keranjang Anda kosong.</p>
            <Link href="/" className="mt-4 inline-block text-[#005D33] font-semibold hover:underline">
              Lihat Paket Menu
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-5 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                <h2 className="font-bold text-sm text-slate-700 flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-[#005D33]" /> Alamat Pengiriman
                </h2>
                <button
                  type="button"
                  onClick={openAddressModal}
                  className="text-xs font-semibold text-[#005D33] hover:underline flex items-center gap-1"
                >
              ...
              {userAddress ? (
                ...
                <div className="px-5 py-4">
                  ...
                  <button type="button" onClick={openAddressModal} className="text-sm text-[#005D33] font-semibold hover:underline mt-1">
                    + Tambah Alamat
                  </button>
                </div>
              )}
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-4 rounded-2xl border-2 text-left transition-all ${paymentOption === 'full' ? 'border-[#005D33] bg-[#005D33]/5' : 'border-slate-100 hover:bg-slate-50'}`}
                  >
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl border-2 text-left transition-all ${paymentOption === 'dp' ? 'border-[#005D33] bg-[#005D33]/5' : 'border-slate-100 hover:bg-slate-50'}`}
                  >
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              ...
              <Button
                ...
                className="w-full h-12 bg-[#005D33] hover:bg-[#004d2a] text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all disabled:opacity-50"
              >
                {isLoading ? (
